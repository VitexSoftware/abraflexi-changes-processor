...
class Engine extends \Ease\SQL\Engine {
    ...
    function processAbraFlexiChanges(array $changes) {
        ...
        foreach ($changes as $change) {
            ...
            if (class_exists($handlerClass)) {
                ...
                if ($this->debug) {
                    $this->addStatusMessage(sprintf(_('Request unexistent module %s for %s'), $handlerClass, $docId), 'warning');
                }
            }
    ...
    public function &gethandler($handlerClass, $id, $changeMeta) {
        if (isset($this->handlerCache[$handlerClass][$id])) {
            ...
            $this->handlerCache[$handlerClass][$id] = new $handlerClass($id, array_merge($changeMeta, ['database' => $this->database]));
    ...
    public function saveLastProcessedVersion($version) {
        ...
        if ($this->debug === true) {
            $this->addStatusMessage(sprintf(_("Last Processed Change ID %s saved for %s"), $version, $this->serverurl()), $result ? 'success' : 'error');
        }
